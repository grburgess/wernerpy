...
wernerpy_dev_log_handler = handlers.TimedRotatingFileHandler(
...
wernerpy_dev_log_handler.setFormatter(_dev_formatter)
wernerpy_dev_log_handler.setLevel(logging.DEBUG)
# now set up the usr log which will save the info

wernerpy_usr_log_handler = handlers.TimedRotatingFileHandler(
    get_path_of_log_file("usr.log"), when="D", interval=1, backupCount=10
)

wernerpy_usr_log_handler.setLevel(logging.INFO)
...
wernerpy_usr_log_handler.setFormatter(_usr_formatter)
...
wernerpy_console_log_handler = logging.StreamHandler(sys.stdout)
wernerpy_console_log_handler.setFormatter(_console_formatter)
wernerpy_console_log_handler.setLevel("INFO")
...
def silence_warnings():
    ...
    wernerpy_usr_log_handler.addFilter(warning_filter)
    wernerpy_console_log_handler.addFilter(warning_filter)
...
def activate_warnings():
    ...
    wernerpy_usr_log_handler.removeFilter(warning_filter)
    wernerpy_console_log_handler.removeFilter(warning_filter)


def update_logging_level(level):

    wernerpy_console_log_handler.setLevel(level)
...
def setup_logger(name):
    ...
    log.addHandler(wernerpy_dev_log_handler)

    log.addHandler(wernerpy_console_log_handler)

    log.addHandler(wernerpy_usr_log_handler)
